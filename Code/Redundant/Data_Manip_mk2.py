...
class graphCreation:
    ...
    def save2DFigSingle(self, varName,yBound=(-20,20),colourMap = "afmhot"):
        ...
        if varName == "dist_fn_x_px_electron":
            dims = self.data.dist_fn_x_px_electron.dims
            localData = self.data.dist_fn_x_px_electron.data
    ...
    def watchSim(self,fileLimit = 50, timeLimit = 108000):
        ...
        while (calendar.timegm(time.gmtime()) - x <timeLimit) and (len(pastList)<(fileLimit+4)):
...
test = graphCreation(fileName="0000.sdf",
                     filePath="epoch-4.17.15/epoch2d/Data/",
                     outputDir="10_laser_decks/92fs")
test.watchSim()
